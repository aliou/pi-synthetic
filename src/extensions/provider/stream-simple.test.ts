...
describe("Synthetic streamSimple wrapper", () => {
  it("detects subscription quotas", () => {
    ...
    expect(isSubscriptionQuotas({})).toBe(false);
    expect(isSubscriptionQuotas(undefined)).toBe(false);
  });

  it("calculates raw pay-as-you-go cost", () => {
    expectCostToEqual(
      calculateSyntheticUsageCost(model, makeMessage().usage, false),
      {
        input: 0.001,
        output: 0.004,
        cacheRead: 0.05,
  ...
  it("applies the subscription cache-read discount", () => {
    expectCostToEqual(
      calculateSyntheticUsageCost(model, makeMessage().usage, true),
      {
        input: 0.001,
        output: 0.004,
        cacheRead: 0.01,
